...
def test_resnetfc():
    key = jax.random.PRNGKey(0)
    x = jnp.ones((20, 10))
    resnetfc = ResnetFC(10, 30)
    params = resnetfc.init(key, x)
    resnetfc.apply(params, x)
...
def test_normalnn():
    ...
    normalnn = NormalNN(10, 30, 3)
    params = normalnn.init(key, x, cats)
    normalnn.apply(params, x, cats)
...
def test_conditionalbatchnorm1d():
    ...
    x = jnp.ones((20, 10))
    y = jnp.ones((20, 1))
    conditionalbatchnorm1d = ConditionalBatchNorm1d(10, 3)
    params = conditionalbatchnorm1d.init(key, x, y)
    conditionalbatchnorm1d.apply(params, x, y)
